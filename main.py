...
from Parametros import edt, t60, d50, c80
...
ri_sint_norm = FiltrodeOctava(ri_sint)
...
ri_sint_filt = func_Conver_log_norm(ri_sint_norm)
...
for data1 in ri_sint_filt:
    ...
    sch_d = Cuadrados_Minimos(sch)
    rta_edt_sint.append(edt(sch_d))
    rta_T60_sint.append(t60(sch_d, 44100))
...
for data1 in respuesta_i_filt:
    ...
    sch_c = Cuadrados_Minimos(sch)
    rta_edt_ri.append(edt(sch_c))
...
for data1 in ri_desc_filt:
    ...
    sch =schroeder(respuesta_i_suave_sint)
    sch_c = Cuadrados_Minimos(sch)
    rta_edt_desc.append(edt(sch_c, ri_desc_fs))
